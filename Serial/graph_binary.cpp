...
Graph::Graph(char *filename, char *filename_w, int type){
  ...
  for (unsigned int i = 0; i < nb_nodes; i++) total_weight += (double)weighted_degree(i);
}
...
Graph::Graph(vector<vector<int> > &c_nodes){
  ...
  for (size_t i = 0; i < c_nodes.size(); i++) nodes.push_back(c_nodes[i]);
}

void Graph::add_node(vector<int> &n){
  nodes.push_back(n);
}
...
void Graph::write_community(const char *filename_w){
  FILE *fOut = fopen(filename_w, "wt");
  for (size_t i = 0; i < nodes.size(); i++){
    fprintf(fOut, "community %zu: ", i+1);
    for (size_t j = 0; j < nodes[i].size(); j++){
...
void Graph::display_reverse(){
  for (unsigned int node = 0; node < nb_nodes; node++){
    ...
    for (unsigned int i = 0; i < nb_neighbors(node); i++){
      if (node > *(p.first + i))
      {
        if (weights.size() != 0) cout << *(p.first + i) << " " << node << " " << *(p.second + i) << endl;
        else cout << *(p.first + i) << " " << node << endl;
      }
    }
  }
}
