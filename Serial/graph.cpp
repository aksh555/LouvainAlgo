...
Graph::Graph(char *filename, int type){
  ...
  while (!finput.eof()){
    ...
    double weight = 1.;
    if (type == WEIGHTED) finput >> src >> dest >> weight;
    else finput >> src >> dest;
    if (finput){
      if (links.size() <= max(src, dest) + 1) links.resize(max(src, dest) + 1);
      links[src].push_back(make_pair(dest, weight));
      if (src != dest) links[dest].push_back(make_pair(src, weight));
      s.insert(src);
      s.insert(dest);
      nb_links++;
    }
...
void Graph::renumber(int type){
  ...
  for (unsigned int i = 0; i < nodes.size(); i++) renum[nodes[i]] = i;
  ...
  for (unsigned int i = 0; i < nodes.size(); i++) nodes[i] = renum[nodes[i]];
...
void Graph::clean(int type){
  for (unsigned int i = 0; i < links.size(); i++){
    ...
    for (unsigned int j = 0; j < links[i].size(); j++){
      ...
      else if (type == WEIGHTED) it->second += links[i][j].second;
    }
...
void Graph::display(int type){
  for (unsigned int i = 0; i < links.size(); i++){
    for (unsigned int j = 0; j < links[i].size(); j++){
      ...
      if (type == WEIGHTED) cout << i << " " << dest << " " << weight << endl;
      else cout << i << " " << dest << endl;
    }
  }
}
