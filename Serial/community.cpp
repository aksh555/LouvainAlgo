...
void Community::init_partition(char *filename){
  ...
  while (!finput.eof()){
    ...
    if (finput){
      ...
      if (i == neigh_last) insert(node, comm, 0);
    }
  }
  finput.close();
}
...
void Community::display(){
  for (int i = 0; i < size; i++) cerr << " " << i << "/" << n2c[i] << "/" << in[i] << "/" << tot[i];
  cerr << endl;
}
...
double Community::modularity(){
  ...
  for (int i = 0; i < size; i++){
    if (tot[i] > 0) q += (double)in[i] / m2 - ((double)tot[i] / m2) * ((double)tot[i] / m2);
  }
  return q;
}
void Community::neigh_comm(unsigned int node){
  for (unsigned int i = 0; i < neigh_last; i++) neigh_weight[neigh_pos[i]] = -1;
  neigh_last = 0;
...
void Community::display_partition(){
  ...
  int final = 0;
  for (int i = 0; i < size; i++)
    if (renumber[i] != -1) renumber[i] = final++;
  for (int i = 0; i < size; i++) cout << i << " " << renumber[n2c[i]] << endl;
}
  ...
  for (int comm = 0; comm < comm_deg; comm++){
    ...
    for (int node = 0; node < comm_size; node++){
      ...
      for (int i = 0; i < deg; i++){
        ...
        if (it == m.end()) m.insert(make_pair(neigh_comm, neigh_weight));
        else it->second += neigh_weight;
      }
    }
...
bool Community::one_level(){
  ...
  for (int i = 0; i < size; i++) random_order[i] = i;
